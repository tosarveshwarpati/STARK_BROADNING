...
larray = [390.5523]
while True:
    method = input("enter the method of backgroung selection: M for Manual and A for autometic:    ")
    if method == 'M':
        class ZoomPan:
            def __init__(self):
                self.press = None
        ...
        for lam in larray:
            ...
            while True:
                ...
                if sel == 'y':
                    ...
                    with open('area'+str(lam)+'_'+str(sample_index)+'.txt', 'w') as f1:
                        f1.write(str(area))
                    with open('correct_out'+str(lam+1)+'_'+str(sample_index)+'.txt', 'w') as f2:
                        f2.write(str(correct_out))
                    param_bounds=([0,0,0,0,0,0.0],[np.inf,np.inf,lambda_max,3,3,1])
                    p = 0.5
                    popt_pv = []
                    ...
                    while True:
                        ...
                        if sel1 == 'y':
                            ...
                            with open('output_data'+str(lam)+'_'+str(sample_index)+'.txt', 'w') as f3:
                                f3.write(str(output_data)) 
                            break
